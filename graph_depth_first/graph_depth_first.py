...
class Graph:
    ...
    def breadth_first(self,start_vertix):
        ...
        return result
def depth_first(graph, start_vertix):
    """
    Perform a depth-first pre-order traversal on a graph.

    Args:
        graph (Graph): The graph to traverse.
        start_vertix (Vertix): The starting vertex.

    Returns:
        list: Pre-order traversal order of vertex values.
    """
    def dfs_recursive(vertix, visited, result):
        visited.add(vertix)
        result.append(vertix.value)
        for edge in graph.get_neighbors(vertix):
            neighbor = edge.vertix
    ...
    result = []
    visited = set()
    dfs_recursive(start_vertix, visited, result)
    return result
